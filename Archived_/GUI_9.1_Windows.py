...
# import create_event  # 暫時不執行 交給 Eden 宣霈
...
class Meeting(tk.Frame):
    ...
    def clickBtnConfirm(self):
        ...
        # 會議與會者頁面
            ...
            self.txtAttendMeeting.grid(row=3, column=1, columnspan = 2, sticky = tk.NE + tk.SW)  # columnspan = 4 (original)
            ...
            self.lblAttendMeeting.grid(row=5, column=0, sticky = tk.NE + tk.SW)
            self.txtAttendMeeting.grid(row=5, column=1, columnspan = 4, sticky = tk.NE + tk.SW)
    ...
    def clickBtnSearch(self):
        ...
        textEntry.set(self.txtAttendMeeting.get("0.0", tk.END))  # TypeError: get() takes 1 positional argument but 3 were given
        fh = open(current_dir + '/meeting.csv', "r") #存取檔案路徑(2)
        csvFile = csv.DictReader(fh)
        for row in csvFile:
            self.m_startDate = row["start date"]
    ...
    def clickBtnJoin(self):
        ...
        self.meeting_name = self.textAttendMeeting.get()  #取會議名稱  blAttendMeeting  AttributeError: 'Label' object has no attribute 'get'
        print ("Información Adicional: {}".format(self.textAttendMeeting.get()))        
    ...
    def clickBtnSubmit(self):
        ...
        #self.meeting_name = self.txtAttendMeeting.get("0.0", tk.END)  #取會議名稱
        print ("Información Adicional: {}".format(self.txtAttendMeeting.get()))
        
        worksheet = sheet.worksheet('title', self.meeting_name)

        input_csv = current_dir + '\Google Calendar API\schedule.csv' 
        df = pd.read_csv(input_csv, encoding = 'big5')  # utf-8_sig
    ...
    def clickBtnCal(self):
        # 從雲端下載所有人填畢的schedule.csv
        meeting_name = 'eden'  # 待修改
        cred_path = current_dir + '\Google Sheet\credentials_ServiceAccount.json'
        output_csv = current_dir +'\schedule_output.csv'
        def_read_write_Gsheet.read_GSheet_to_csv(meeting_name, cred_path, output_csv)
        
        # 拿取不重複的email  # 改路徑方式!
        self.data = pd.read_csv(output_csv) #存取檔案路徑(1)  #host的存取
...
def check_date_range(start_date, end_date):
    ...
    meeting_name = 'eden' #  待修改
    Gsheet_date = def_read_write_Gsheet.sendback_date(meeting_name)
    start_date = Gsheet_date[0]
    end_date = Gsheet_date[1]
