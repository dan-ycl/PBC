...
class Meeting(tk.Frame):
    ...
    def clickBtnBuild(self):        
        ...
        if check_empty_textbox(text_list) is False or check_empty_combobox(entry_list) is False: # 自建函數
            ...
            self.m_title = self.txtTitle.get("0.0", tk.END).rstrip()  # 移除字尾斷行
            self.m_location = self.txtLocation.get("0.0", tk.END).rstrip()
    ...
    def clickBtnSearch(self):
        ...
        self.m_startTime = cloud_meeting[2]
        self.m_endTime = cloud_meeting[3]
    ...
    def clickBtnJoin(self):
        self.name = self.txtAttendant.get("0.0", tk.END).rstrip()  # 移除字尾斷行
        self.email = self.txtEmail.get("0.0", tk.END).rstrip()
    ...
    def clickBtnCal(self):
        ...
        self.data['email'] = self.data['email'].str.rstrip()
